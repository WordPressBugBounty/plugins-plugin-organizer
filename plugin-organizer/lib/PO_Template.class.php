	...
	function get_post_meta_box($post) {
		...
		if ($post->ID != "" && is_numeric($post->ID)) {
			...
			if ($postSettings === null) {
				$postSettings = array('children'=>'', 'pt_override'=>'', 'secure'=>'', 'post_priority'=>'');
			}
			...
			foreach ($availableRoles as $roleID=>$roleName) {
				...
				if ($storedPluginLists == null) {
					$storedPluginLists = array('disabled_plugins'=>'', 'enabled_plugins'=>'', 'disabled_mobile_plugins'=>'', 'enabled_mobile_plugins'=>'', 'disabled_groups'=>'', 'enabled_groups'=>'', 'disabled_mobile_groups'=>'', 'enabled_mobile_groups'=>'');
				}
