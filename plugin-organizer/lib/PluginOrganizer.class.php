...
class PluginOrganizer {
	...
	var $regex, $absPath, $urlPath, $nonce, $ajax, $tpl, $pluginDirPath, $pluginBase, $siteURL;
	private $postedData;
	...
	function check_version() {
		...
		if (get_option("PO_version_num") != "10.2.5" && !in_array($pagenow, array("plugins.php", "update-core.php", "update.php"))) {
			$this->activate();
		}
	}
	...
	function activate() {
		...
		if (get_option("PO_version_num") != "10.2.5") {
			update_option("PO_version_num", "10.2.5");
		}
	...
	function fix_trailng_slash($permalink) {
		...
		if (is_object($row)) {
			...
			if ($this->siteURL == preg_replace('/\/$/', '', $realSiteUrl)) {
